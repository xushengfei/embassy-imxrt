    ...
    pub fn set_peripheral(&self, peripheral: PeripheralType, lock: bool) -> bool {
        if peripheral != PeripheralType::NoPeriphSelected {
            ...
            } else if peripheral == PeripheralType::Usart && self.regs().pselid().read().usartpresent().is_not_present()
            {
                return false;
                //todo add a panic here
            } else if (peripheral == PeripheralType::I2sReceive || peripheral == PeripheralType::I2sTransmit)
                && self.regs().pselid().read().i2spresent().is_not_present()
            {
                return false;
                //todo add a panic here
            }
...
pub struct Uart {
    ...
    interruptPri: u16, //dma: dma0::RegisterBlock,
...
impl Uart {
    ...
    fn uart_calculate_baud_rate(&self, _clkFreq_Hz: u32) {
        ...
        // TODO: add code to calculate best osrval and best brg val.
